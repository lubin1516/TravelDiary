...
public class MImageLoader {
	...
	private static File directory() {
		...
		return new File(path,"imageloadercache2");
	}
	/**
	 * 获取url指向的图像，放到iv中显示
	 * @param url 图像路径
	 ...
	 */
	public static void loadImage(final String url,final ImageView iv,final int height,final int width){
		if(isFirstTime){
	...
	public static Bitmap compress(InputStream is, ImageView iv, int THeight, int TWidth) {
		Bitmap bitmap = null;
		try{
			...
			//2)获取希望的宽度/高度
			int targetWidth  = dip2px(CustomApplcation.getInstance(),TWidth);
			int targetHeight = dip2px(CustomApplcation.getInstance(),THeight);
